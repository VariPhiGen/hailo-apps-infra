...
class GStreamerApp:
    def __init__(self, args, user_data: app_callback_class):
        ...
        self.video_sink = "fakesink" if self.options_menu.disable_display else GST_VIDEO_SINK
        self.pipeline = None
        self.loop = None
        self.threads = []
        self.error_occurred = False
        self.pipeline_latency = 50  # milliseconds
    ...
    def run(self):
        ...
        if not self.options_menu.disable_callback:
            original_identity = self.pipeline.get_by_name("original_frame_identity")
            print(original_identity)
            if original_identity is None:
                print("Warning: original_frame_identity element not found, add <identity name=original_frame_identity> in your pipeline where you want the callback to be called.")
            else:
                original_identity_pad = original_identity.get_static_pad("src")
                original_identity_pad.add_probe(Gst.PadProbeType.BUFFER, self.app_callback, self.user_data, "original")
            ...
            if identity is None:
                ...
                identity_pad.add_probe(Gst.PadProbeType.BUFFER, self.app_callback, self.user_data,"processing")
